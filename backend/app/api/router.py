...
def update_oven_hours(oven_id: int, body: OvenHoursUpdate, db: Session = Depends(get_db)):
    ...
    if body.open_min >= body.close_min:
        raise HTTPException(400, "打烊分钟必须晚于开门分钟")
...
def create_batch(body: BatchCreate, db: Session = Depends(get_db)):
    ...
    if not fits_operating_hours(candidates, oven.open_min, oven.close_min):
        span_start = min(o.interval.start for o in candidates)
        span_end = max(o.interval.end for o in candidates)
        detail = (
            f"超出该炉营业时段：占炉 [{span_start},{span_end}) 探出 "
            f"{oven.label} 营业时段 [{oven.open_min},{oven.close_min})（打烊点不可排）"
        )
